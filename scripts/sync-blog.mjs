...
function verifyOrigin() {
	...
	if (!fetchUrl || normalizeRemoteUrl(fetchUrl) !== EXPECTED_REMOTE) {
		console.error(
			"同步已停止：origin 的 fetch URL 不是 github.com/17356085/17356085.github.io。",
		);
		return false;
	}
...
function requireCleanWorktree(context) {
	...
	const blockedOperation =
		context === "fetch" ? "fetch 或写操作" : "fast-forward 写操作";
	console.error(
		`同步已停止：${context}前发现工作区存在 tracked/untracked 修改；未执行 ${blockedOperation}。`,
	);
...
function main() {
	...
	if (counts.ahead > 0 && counts.behind === 0) {
		console.error(
			`同步已停止：本地领先 origin/main；local-only=${counts.ahead}，remote-only=0。未自动 push。`,
		);
		return 1;
	}
	...
	if (counts.ahead > 0 && counts.behind > 0) {
		console.error(
			`同步已停止：本地与 origin/main 已分叉；local-only=${counts.ahead}，remote-only=${counts.behind}。未自动合并。`,
		);
		return 1;
	}
...
try {
	...
	console.error(
		`同步已停止：${error instanceof Error ? error.message : String(error)}`,
	);
	process.exitCode = 1;
}
