...
带笔写作时不需要绕过文章再手动经过 PicGo，可以直接调用仓库的上传脚本：

```bash
pnpm media:upload -- <图片路径> --category posts --json
```
...
对象名使用内容哈希，例如 `images/posts/<md5>.<ext>`。新的上传流程禁止生成 `images/YYYY/MM/`，也不要在仓库中重新堆积正文图片和封面。
